    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    state["step"] = 0
                    state["square_avg"] = torch.zeros_like(p.processing_time)
                    if group["momentum"] > 0:
                        state["momentum_buffer"] = torch.zeros_like(p.processing_time)
                    if group["centered"]:
                        state["grad_avg"] = torch.zeros_like(p.processing_time)
                ...
                if group["weight_decay"] != 0:
                    grad = grad.add(group["weight_decay"], p.processing_time)
                ...
                if group["momentum"] > 0:
                    ...
                    p.processing_time.add_(-group["lr"], buf)
                else:
                    p.processing_time.addcdiv_(-group["lr"], grad, avg)
