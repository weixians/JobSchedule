...
class AttributeSavingMixin(object):
    ...
    def load(self, dirname: str, device=None) -> None:
        """Load internal states."""
        self.__load(dirname, [], device)

    def __load(self, dirname: str, ancestors: List[Any], device) -> None:
        ...
        for attr in self.saved_attributes:
            ...
            if isinstance(attr_value, AttributeSavingMixin):
                ...
                if isinstance(
                    ...
                    attr_value = attr_value.module
                attr_value.load_state_dict(torch.load(os.path.join(dirname, "{}.pt".format(attr)), device))
        ancestors.pop()
