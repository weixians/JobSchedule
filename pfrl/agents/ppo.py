...
def _add_log_prob_and_value_to_episodes_recurrent(episodes, model, phi, batch_states, obs_normalizer, device):
    ...
    with torch.no_grad(), pfrl.utils.evaluating(model):
        ...
        flat_actions = torch.from_numpy(np.array([b["action"] for b in flat_transitions])).to(device)
...
def _add_log_prob_and_value_to_episodes(episodes, model, phi, batch_states, obs_normalizer, device):
    ...
    with torch.no_grad(), pfrl.utils.evaluating(model):
        ...
        actions = torch.from_numpy(np.array([b["action"] for b in dataset])).to(device)
...
def _make_dataset_recurrent(
    episodes, model, phi, batch_states, obs_normalizer, gamma, lambd, max_recurrent_sequence_len, device
):
...
class PPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(dataset, minibatch_size=self.minibatch_size, num_epochs=self.epochs):
            ...
            actions = torch.from_numpy(np.array([b["action"] for b in batch])).to(device)
